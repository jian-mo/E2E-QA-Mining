...
class SquadMultitask(nlp.GeneratorBasedBuilder):
    ...
    def process_aeqg(self, paragraph):
        source_text = f"generate questions and answers: {paragraph['context'].strip()}"
        questions_answers = [qas['question'].strip()+" "+qas['answers'][0]['text'] for qas in paragraph['qas']]
        target_text = " {sep_token} ".join(questions_answers)
        target_text = f"{target_text} {{sep_token}}"
        return {"source_text": source_text, "target_text": target_text, "task": "aeqg"}
    ...
    def _generate_examples(self, filepath):
        ...
        count = 0
        tasks = ['qa', 'qg', 'ans_ext', 'e2e_qg','aeqg']
        with open(filepath) as f:
            ...
            for article in squad["data"]:
                ...
                for paragraph in article["paragraphs"]:
                    ...
                    if 'aeqg' in tasks:
                        yield count, self.process_aeqg(paragraph)
                        count += 1
